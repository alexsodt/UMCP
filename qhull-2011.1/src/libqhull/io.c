/*<html><pre>  -<a                             href="qh-io.htm"
  >-------------------------------</a><a name="TOP">-</a>
   ...
   $Id: //main/2011/qhull/src/libqhull/io.c#2 $$Change: 1342 $
...
extern int ntri;
extern int ntri_space;
extern int nridges;
extern int nverts;
extern int write_to_globals;
extern int nridge_space;
extern int nvert_space;
...
void qh_produce_output2_writeglobals(void) {
  int i, tempsize= qh_setsize(qhmem.tempstack), d_1;
  
    qh_printfacets_to_globals(qh facet_list, NULL, !qh_ALL);
}
  ...
  if (qh CENTERtype == qh_ASvoronoi) {
    ...
    if (!facet->normal || !facet->upperdelaunay || !qh ATinfinity) {
      if (!facet->center)
       ...
       if( write_to_globals )
	{ 
		if( nverts >= nvert_space )
		{
			if( nvert_space == 0 || vertStorage == NULL )
			{
				nvert_space = 100;
			...
			{
				nvert_space *= 2;
				vertStorage = (double *)realloc( vertStorage, sizeof(double) * 3 * nvert_space );
			}
		}
		
			vertStorage[3*nverts+0] = facet->center[0];
			vertStorage[3*nverts+1] = facet->center[1];
			if( qh hull_dim == 2 ) 
				vertStorage[3*nverts+2] = 0;
			else
				vertStorage[3*nverts+2] = facet->center[2];
			nverts++;
...
void qh_printfacets_to_globals(facetT *facetlist, setT *facets, boolT printall) {
  int in_numfacets, numsimplicial, numridges, totneighbors, numcoplanars, numtricoplanars;
  coordT *center;
  realT outerplane, innerplane;
...
void qh_printvridge(FILE *fp, vertexT *vertex, vertexT *vertexA, setT *centers, boolT unbounded) {
  ...
  if( write_to_globals && (nridges >= nridge_space) )
  {
     if( ridgeStorage == NULL )
     {
		nridge_space = 100;
		ridgeStorage = (ridge *)malloc( sizeof(ridge) * nridge_space );
     }
     else
     {
		nridge_space *= 2;
		ridgeStorage = (ridge *)realloc( ridgeStorage, sizeof(ridge) * nridge_space );
     }
  }

  if( write_to_globals )
  {
	ridgeStorage[nridges].nvertices = qh_setsize(centers);
	ridgeStorage[nridges].site1 = qh_pointid(vertex->point);
	ridgeStorage[nridges].site2 = qh_pointid(vertexA->point);
        ridgeStorage[nridges].vertices = (int *)malloc( sizeof(int) * ridgeStorage[nridges].nvertices );
  }
  ...
  FOREACHfacet_(centers)
  {
    if( write_to_globals )
    {
	ridgeStorage[nridges].vertices[x] = facet->visitid;
	x++;
    }
    qh_fprintf(fp, 9276, " %d", facet->visitid);
  }

  if( write_to_globals) nridges++;
  qh_fprintf(fp, 9277, "\n");
} /* printvridge */
