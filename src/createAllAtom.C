...
//						printf("%lf %lf evaluates to %d %lf %lf %s\n", source_r[0], source_r[1], fout, spot_u, spot_v, (regions_for_face[fout]== r ? "INSIDE" : "OUTSIDE") ); 
					...
					if( fout == debug_f && x_leaflet == 1 && pass == 1)
					{
						...
						for( int xa = lipid_start[l]; xa <= lipid_stop[l]; xa++ )
						{
							...
							if( xa == lipid_start[l] && debug_me && x_leaflet == 1)
							{
								if( l == 41 )
								{
									printf("right here.\n");
								}
								printf("H %lf %lf %lf lipid %d seg %s\n", eval[0], eval[1], eval[2], l, cur_segname );
							}
#endif
							debug_on = 0;
...
//							if( xa == lipid_start[l] && x_leaflet == 1 )
//								printf("%c at %lf %lf %lf segment %s\n", at[xa].atname[0], at[xa].x, at[xa].y, at[xa].z, at[xa].segid );  
