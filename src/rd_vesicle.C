...
//	srand(1);	
	...
	double sphere_rad = sqrt(area0/(4*M_PI));
	...
	//		printf("Current time step %le nanoseconds.\n", corr_step * time_step * block.kinetic_corr_period *(1e9) );
			...
			for( int t = 0; t < block.o_lim; t++, cur_t += time_step )
			{
				...
				double Dtot = (block.diffc + block.aqueous_diffc);
					...
					for( int p = 0; p < aqueous_np; p++ )
					{
						...
						while( !done )
						{
							...
							if( r_move > distance[p] )
							{
								...
								double rad = sub_surface->returnRadius( aqueous_r+3*p, &f, &u, &v,  M, mlow, mhigh,  r_move, distance[p],  -1,  defaultR);
