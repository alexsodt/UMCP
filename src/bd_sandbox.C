...
*/
    ...
    while( close )
	{
		close = false;
	    for (int i = 0; i < num_part; i++) {
	        data[i].x = rand() % 180 - 90;
	        data[i].y = rand() % 180 - 90;
	        data[i].z = rand() % 180 - 90;
...
#ifdef REDO_PARTICLES
		while( outside )
		{
			data[i].x = -LA/2 + LA * rand() / (double)RAND_MAX;	
			data[i].y = -LB/2 + LB * rand() / (double)RAND_MAX;	
			data[i].z = -LC/2 + LC * rand() / (double)RAND_MAX;	
		
	
			double pt[3] = { data[i].x, data[i].y, data[i].z };
			int f;
			double u,v;
			
			if( theSurface1->withinBoxedSurface( pt, &f, &u, &v, M, mlow, mhigh, distance+i,L ) )
			{
				outside = 0; 
...
//			if( theSurface1->withinSurface( pt, &f, &u, &v, M, mlow, mhigh, distance+i) )
			ncalls++;
		}
	//	printf("Made particle %d/%d in %d calls.\n", i, num_part, ncalls );
#else
		distance[i] = radius;
#endif
	}
	...
	bool allIn = true;
		for( int j = 0; j < num_part; j++ )
		{
		    ...
		    if (changeInDistance >= distance[j]) {
                        ...
                        int f;
                        double u, v, radius;
			radius = theSurface1->returnRadius(conversion(data[j]), &f, &u, &v,  M, mlow, mhigh,  changeInDistance, distance[j],  L, defaultR);
			...
			if (radius > 0) {
				distance[j] = radius;
                        	...
                        	allIn = false;
		//		printf("%lf %lf \n", distance[j], changeInDistance);
                		//printf("one fell out: %lf %lf %lf %lf %lf %lf\n", data[j].x, data[j].y, data[j].z, temp_data[j].x, temp_data[j].y, temp_data[j].z);    
