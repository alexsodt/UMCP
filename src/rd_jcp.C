...
//	sub_surface->generateSubdivisionMatrices( M, mlow, mhigh );
	...
	newSurface->theSurface = sub_surface;
	newSurface->id = 0;
	sub_surface->surface_id = newSurface->id;
	...
	for( int x = 0; x < 3; x++ )
	for( int y = 0; y < 3; y++ )
		theSimulation->PBC_vec[x][y] = sub_surface->PBC_vec[x][y];
	...
	newSurface->g = g;
	newSurface->nc = nc;
	newSurface->theForceSet = theForceSet;
				...
				if( block.mean_field )
				{
					...
					for( int ap = 0; ap < aqueous_np; ap++ )
					{
						...
						else if( distance[ap] < Rmax )
						{
							...
							double k;
							double c_tot = sub_surface->c(sf, su, sv, r,&k );
...
//				printf("ncind: %d ncompl %le\n", ncind, npaired );
			...
			if( block.tachyon)
			{
				...
				for( int i = 0; i < aqueous_np; i++ )
				{
					...
					temp_complexes[i]->refresh(theSimulation);
