...
int main( int argc, char **argv )
{
	if( argc < 3 )
	{	
		printf("Syntax: makePlanar NX Length [Lz] [target Ly]\n");
		return 0;
	}

	int nx_int = atoi(argv[1]);
	int ny_int;
	PeriodicLengthA = atof(argv[2]);
...
/*		else
	...
	sprintf(fileName,"planar.mesh");

	FILE *theLattice = fopen(fileName,"w");

	if( Lc > 0 )
		fprintf(theLattice, "3D supplemented lattice\n" );
	else
		fprintf(theLattice, "supplemented lattice\n" );
	...
	for( ; p < npoints; p++ )
	{
		...
		fprintf(theLattice, "%d %lf %lf %lf %d", pt_map[p], r[3*p+0], r[3*p+1], 0.0 , n_new_bonds[p] );
