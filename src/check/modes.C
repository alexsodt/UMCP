...
*/
	...
	if( vq_m1 && pmesh_m1 )
	{
		for( int vx = 0; vx < par_info.nv; vx++ )
		{
			...
			alt_T += 0.5 * (pmesh[3*v+0]+pmesh_m1[3*v+0])/2 * (vq[3*v+0]+vq_m1[3*v+0])/2/(alphas[0]*alphas[0]);
			alt_T += 0.5 * (pmesh[3*v+1]+pmesh_m1[3*v+1])/2 * (vq[3*v+1]+vq_m1[3*v+1])/2/(alphas[1]*alphas[1]);
			alt_T += 0.5 * (pmesh[3*v+2]+pmesh_m1[3*v+2])/2 * (vq[3*v+2]+vq_m1[3*v+2])/2/(alphas[2]*alphas[2]);
		}
	}
	else
	{
		...
		{
			int v = par_info.verts[vx];
			alt_T += 0.5 * pmesh[3*v+0] * vq[3*v+0]/(alphas[0]*alphas[0]);
			alt_T += 0.5 * pmesh[3*v+1] * vq[3*v+1]/(alphas[1]*alphas[1]);
			alt_T += 0.5 * pmesh[3*v+2] * vq[3*v+2]/(alphas[2]*alphas[2]);
		}
	}
#ifdef PARALLEL
	ParallelSum(&alt_T,1);
