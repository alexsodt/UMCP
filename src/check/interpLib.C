...
void MinImage( double *dr1, double PBC_vec[3][3], double *put_vec )
{
	...
	while( !done )
	{
		...
		{
			tr[0] += dx * PBC_vec[0][0] + dy * PBC_vec[1][0] + dz * PBC_vec[2][0];
			tr[1] += dx * PBC_vec[0][1] + dy * PBC_vec[1][1] + dz * PBC_vec[2][1];
			tr[2] += dx * PBC_vec[0][2] + dy * PBC_vec[1][2] + dz * PBC_vec[2][2];
...
int surface::loadLattice( const char *fileName, double noise, surface *copyFrom )
{
	...
	for( int t = 0; t < nt; t++ )
	{
		...
		MinImage( dr1, PBC_vec, theTriangles[t].pbc1 );
		MinImage( dr2, PBC_vec, theTriangles[t].pbc2 );
		double o[3] = {0,0,0};
...
//				printf("sw: %d\n", sw );
	...
	for( int t = 0; t < nt; t++ )
	{
		...
		double put[3] = {0,0,0};
		MinImage( dr1, PBC_vec, put );
		put[0] = 0;
		put[1] = 0;
		put[2] = 0;
		MinImage( dr2, PBC_vec, put );
	...
	constructTriangles();
	writeXYZSurface("pre_remove.xyz","pre_remove.psf", this );
	constructTriangles();
//	writeXYZSurface("init.xyz","init.psf", this );
	writeXYZandPSFPeriodic("init");
...
//				theVertices[i].nrm[0], theVertices[i].nrm[1], theVertices[i].nrm[2], dp ); 
	...
	{	
		for( int i = 0; i < nv; i++ )
		{
			double *ri = theVertices[i].r;
	
			double phi[MAX_VALENCE];
	
			double nrm[3];
	
			memcpy( nrm, theVertices[i].nrm, sizeof(double) * 3 );
			...
			double dp = vec1[0] * nrm[0] + vec1[1] * nrm[1] + vec1[2] * nrm[2];
			...
			cross( vec1, vec2, vcheck );
	
			double dp_n = vcheck[0] * nrm[0] + vcheck[1] * nrm[1] + vcheck[2] * nrm[2];
	
			if( dp_n < 0 )
			{
				double temp[3];
				memcpy( temp, vec1, sizeof(double) * 3 );
				memcpy( vec1, vec2, sizeof(double) * 3 );
				memcpy( vec2, temp, sizeof(double) * 3 );
			}
			int broken_index = -1;
			double broken_phi = 0;
			for( int v = 0; v < theVertices[i].valence; v++ )
			{
	
				double *r = theVertices[theVertices[i].edges[v]].r;
				double dr[3] = { r[0] - ri[0] + theVertices[i].edge_PBC[3*v+0]*PBC_vec[0][0] + theVertices[i].edge_PBC[3*v+1] * PBC_vec[1][0]+ theVertices[i].edge_PBC[3*v+2] * PBC_vec[2][0], 
						 r[1] - ri[1] + theVertices[i].edge_PBC[3*v+0]*PBC_vec[0][1] + theVertices[i].edge_PBC[3*v+1] * PBC_vec[1][1]+ theVertices[i].edge_PBC[3*v+2] * PBC_vec[2][1], 
						 r[2] - ri[2] + theVertices[i].edge_PBC[3*v+0]*PBC_vec[0][2] + theVertices[i].edge_PBC[3*v+1] * PBC_vec[1][2]+ theVertices[i].edge_PBC[3*v+2] * PBC_vec[2][2] };
	 
				double dp1 = dr[0] * vec1[0] + dr[1] * vec1[1] + dr[2] * vec1[2];
				double dp2 = dr[0] * vec2[0] + dr[1] * vec2[1] + dr[2] * vec2[2];
	
				phi[v] = atan2( dp2, dp1 );	
	
				if( theVertices[i].broken[v] )
				{
					broken_index = v;
					broken_phi = phi[v];
				}
			...
			if( pass == 1 && broken_index >= 0 )
			{
				for( int v = 0; v < theVertices[i].valence; v++ )
				{
					phi[v] = 2*M_PI + phi[v] - broken_phi;
	
					if( v != broken_index )
					{
						while( phi[v] >= 2*M_PI ) phi[v] -= 2*M_PI;
					}
				}
			}
			...
			if( broken_index >= 0 )
			{
				printf("broken_index is %d.\n", broken_index);
				for( int x = 0; x  <theVertices[i].valence;x++)
					printf(" %lf", phi[x] );
				printf("\n");
			}

			int sorter[MAX_VALENCE];
			...
			while( !done)
			{
				...
				for( int t = 0; t < theVertices[i].valence-1; t++ )
					if( phi[sorter[t]] > phi[sorter[t+1]] )
					{
						done = 0;
						int x = sorter[t];
						sorter[t] = sorter[t+1];
...
//			printf("sorting %d val: %d ", i, theVertices[i].valence  );
//			for( int x = 0; x < theVertices[i].valence; x++ )
//				printf(" %d ", sorter[x] ); 
			if( pass == 1 )
			{
//				printf("broken:");
//				for( int x = 0; x < theVertices[i].valence; x++ )
//					printf(" %d ", (theVertices[i].broken[sorter[x]] ? 1 : 0) ); 
			}
//				printf("\n");
			int oe[MAX_VALENCE];
			memcpy( oe, theVertices[i].edges, sizeof(int) * theVertices[i].valence );
...
void surface::assignEdgePBC( void )
{
	for( int i = 0; i < nv; i++ )
	{
		...
		for( int e = 0; e < val; e++ )
		{
			...
			MinImage( dr, PBC_vec, theVertices[i].edge_PBC+3*e );
		}
	}
}
...
int surface::createEdge(int i, int j, int k )
{
	...
	MinImage( dr, PBC_vec, theVertices[i].edge_PBC+3*(val) );
	...
	MinImage( dr2, PBC_vec, theVertices[k].edge_PBC+3*(val) );
...
//					printf("broken: i: %d %d %d %d i: %d\n", i, j, k, theVertices[k].edges[ekp1], theVertices[k].edges[ek] );
...
#if 0 // I think I can do better than this: check to see if the edge is connect to a sense..
						if( orientation_problem[next_pt] || theVertices[cur_pt].broken_chain[cur_edge] )
						{
							if( orientation_problem[next_pt]) 
							{
								printf("loop");
								for( int x = 0; x < loop_size[nloops]; x++ )
									printf(" %d", loops[nloops*max_loop+x] );
								printf(" ending at unorientable index %d.\n", next_pt ); 
								term_with = next_pt;
							}
							is_loop[nloops] = 0;
							bad_loop = 1;
		...
		{	
			...
			for( int okay_s = 0; okay_s < loop_size[l]-3; okay_s++ )
			{
				int i = loops[l*max_loop+okay_s+0];
				int j = loops[l*max_loop+okay_s+1];
				int k = loops[l*max_loop+okay_s+2];
	
				/*if( loop_size[l] > 3 || !is_loop[l] )
				{
					if( createEdge( i, j, k ) )
					{
						edge_created = 1;
	...
	for( int vz = 0; vz < dupez; vz++ )
	{
		for( int v = 0; v < copy_surface->nv; v++ )
		{
			...
			for( int xx = 0; xx < copy_surface->theVertices[v].valence; xx++ )
			{
				...
				MinImage( dr, copy_surface->PBC_vec, add );
...
void surface::wrapPBC( double *dr1 )
{
	double put[3];
	MinImage( dr1, PBC_vec, put );
