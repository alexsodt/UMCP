...
void pcomplex::propagate_q( surface *theSurface, double *rsurf, double dt )
{
	// move a step.
...
/*	
		...
		PBC_ext[3*s+2] = del[2];
*/		
		rall[3*s+0] += theSurface->PBC_vec[0][0] * PBC_ext[3*s+0] + theSurface->PBC_vec[1][0] * PBC_ext[3*s+1] + theSurface->PBC_vec[2][0] * PBC_ext[3*s+2];
		rall[3*s+1] += theSurface->PBC_vec[0][1] * PBC_ext[3*s+0] + theSurface->PBC_vec[1][1] * PBC_ext[3*s+1] + theSurface->PBC_vec[2][1] * PBC_ext[3*s+2];
		rall[3*s+2] += theSurface->PBC_vec[0][2] * PBC_ext[3*s+0] + theSurface->PBC_vec[1][2] * PBC_ext[3*s+1] + theSurface->PBC_vec[2][2] * PBC_ext[3*s+2];
		...
		{
			update_attach = 0;
			center_on[0] = rall[3*s+0];
			center_on[1] = rall[3*s+1];
			center_on[2] = rall[3*s+2];
		}

		double dr[3] = { rall[3*s+0] - center_on[0], rall[3*s+1] - center_on[1], rall[3*s+2] - center_on[2] };

		theSurface->wrapPBC( dr );

		rall[3*s+0] = center_on[0] + dr[0];
		rall[3*s+1] = center_on[1] + dr[1];
		rall[3*s+2] = center_on[2] + dr[2];
#endif
	}
...
{
	memset( particle_g, 0, sizeof(double) * 3 * nsites );
	...
	for( complex_record *rec = block->complex_records; rec; rec = rec->next )
	{
		...
		for( int p = 0; p < nsol_tot; p++ )
		{
			...
			while( !io_check )
			{
				...
				tp[0] = f1 * PBC_vec[0][0] + f2 * PBC_vec[1][0] + f3 * PBC_vec[2][0];
				tp[1] = f1 * PBC_vec[0][1] + f2 * PBC_vec[1][1] + f3 * PBC_vec[2][1];
				tp[2] = f1 * PBC_vec[0][2] + f2 * PBC_vec[1][2] + f3 * PBC_vec[2][2];
...
double pcomplex::T( void )
{
	...
	{
		// T = 0.5 * p * qdot
		T += 0.5 * (p[2*s+0]) * (qdot[2*s+0]);
		T += 0.5 * (p[2*s+1]) * (qdot[2*s+1]);
...
void pcomplex::loadCoords( surface *theSurface, double *rsurf, double *r, double *n )
{
	if( bound )
	{
		for( int s = 0; s < nattach; s++ )
		{
			...
			r[3*s+0] += theSurface->PBC_vec[0][0] * PBC_ext[3*s+0] + theSurface->PBC_vec[1][0] * PBC_ext[3*s+1] + theSurface->PBC_vec[2][0] * PBC_ext[3*s+2];
			r[3*s+1] += theSurface->PBC_vec[0][1] * PBC_ext[3*s+0] + theSurface->PBC_vec[1][1] * PBC_ext[3*s+1] + theSurface->PBC_vec[2][1] * PBC_ext[3*s+2];
			r[3*s+2] += theSurface->PBC_vec[0][2] * PBC_ext[3*s+0] + theSurface->PBC_vec[1][2] * PBC_ext[3*s+1] + theSurface->PBC_vec[2][2] * PBC_ext[3*s+2];
