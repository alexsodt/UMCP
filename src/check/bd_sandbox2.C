...
//	read_data(input);
	...
	for (int i = 0; i < num_part; i++) {
		int f;
		double u, v, radius;
		//bool check = theSurface1->withinSurface( conversion(data[i]), &f, &u, &v, M5, M6, &radius );
		bool check2 = theSurface1->withinBoxedSurface( conversion(data[i]), &f, &u, &v, M5, M6, &radius, L );
		printf("%d: %lf\n", i,  radius);
	...
	bool allIn = true;
		for( int j = 0; j < num_part; j++ )
		{
		    ...
		    if (changeInDistance >= distance[j]) {
                        ...
                        int f;
                        double u, v, radius;
                        //bool check = theSurface1->withinSurface( conversion(data[j]), &f, &u, &v, M5, M6, &radius );
                        bool check = theSurface1->withinBoxedSurface( conversion(data[j]), &f, &u, &v, M5, M6, &radius, L );
			if (check) {
				distance[j] = radius;
		...
		//		printf("%lf %lf \n", distance[j], changeInDistance);
		    ...
		    } else if (changeInDistance-distance[j] > 0) {
				printf("ATTENTIONNNNNNNNNNN NOT GOOD");
			} 
...
/**			int f;
			double u, v, radius;
//			printf("before check");
			bool check = theSurface1->withinSurface( conversion(data[j]), &f, &u, &v, M5, M6, &distance );
//			printf("after check");
			if (check) {
				printf("the point %d is within the surface\n", j);
			} else {
				allIn = false;
				printf("the point %d is NOT within the surface\n", j);
				write_data(file3);
				//i--;
				//t -= del_t;
				//delete[] data;
